...
- 커버플로우는 뷰포트의 가로/세로 크기를 함께 고려하여 최소 200px 이상으로 동적으로 표시한다.
...
- 중앙 콘텐츠
  ...
  - 트랙 정보 표시 오버레이
    ...
    - 재생/일시정지 버튼
      ...
      - previewUrl 재생 준비 중에는 버튼을 로딩 상태(스피너)로 표시하고, 중복 클릭을 방지하기 위해 버튼 입력을 비활성화한다.
      - 처음 재생 시 볼륨 0에서 시작하여 짧은 페이드 인을 적용한다.
      - 재생 중 커버플로우에서 다른 앨범(트랙)을 선택하면 기존 트랙을 짧게 페이드 아웃하고, 선택한 트랙을 바로 이어서 재생한다.
