...
- `/embed/[handle]` — iframe 임베드용 페이지 (공개)
...
- 중앙 콘텐츠
  ...
  - 플레이리스트
    ...
    - 하단에 "Embed" 버튼
      - 클릭 시 임베드 모달 오픈
      - 임베드 링크 (`/embed/{handle}`) 와 iframe HTML 코드를 제공
      - 각각 "Copy" 버튼으로 클립보드에 복사 가능
      - 미리보기 iframe 표시

### 임베드 페이지

iframe 으로 삽입할 수 있는 플레이리스트 페이지. (`/embed/[handle]`)
공개 접근 가능하며, 헤더/네비게이션 없이 콘텐츠만 표시한다.

- iframe 크기에 따른 반응형 레이아웃:
  - small (높이 < 200px 또는 너비 < 250px): 앨범 커버 + 트랙명/아티스트명 + 재생 버튼 (한 줄 레이아웃)
  - medium (높이 < 400px 또는 너비 < 400px): 커버플로우 + 트랙 정보 (컴팩트)
  - large (그 외): 유저 핸들 표시 + 커버플로우 + 트랙 정보 + Apple Music 링크
- 재생/일시정지 버튼 (previewUrl 이 있는 경우에만 표시)
- 검색 엔진 인덱싱 방지 (`robots: noindex`)
